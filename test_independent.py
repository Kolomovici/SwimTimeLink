...
def test_key_press(key):
    ...
    if key == 4:
        result = shared_functions.start_timing()
        if result:
            print(f"✅ 开始计时成功，返回值: {result}")
        else:
            print("错误：开始计时失败")
            
    elif key == 5:
        result = shared_functions.test_latency()
        if result is not None:
            print(f"✅ 延迟测试完成，延迟: {result:.3f} ms")
        else:
            print("错误：测试延迟失败")
...
def run_single_test(test_name, df, verbose=True):
    ...
    try:
        ...
        # 保存错误日志 - 修改保存路径
        error_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_result")
...
def run_comprehensive_test():
    ...
    # 生成汇总报告
    report_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_result")
